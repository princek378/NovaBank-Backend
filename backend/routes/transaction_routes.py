...
def get_owner_status(account):
    """Return the status of the user who owns this account."""
    if not account:
        return None
...
def deposit():
    ...
    if is_frozen(account):
        return jsonify(
            {"message": "This account is frozen. Transactions are not allowed."}
        ), 403
...
def transfer():
    ...
    receiver_number = data.get("to_account")
    amount = float(data.get("amount", 0) or 0)
    ...
    if is_frozen(receiver):
        return jsonify(
            {"message": "Receiver account is frozen. Transfer not allowed."}
        ), 403
